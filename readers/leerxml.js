...
const partidosnacional = async () => {
    let my_content;
    try {
      const json = await readFile("./files/AVA_CJ_00_0028_9031.xml");
...
const partidosdepartamentos = async () => {
    let my_content;
    try {
      const json = await readFile("./files/AVA_CJ_DE_0028_9031.xml");
      my_content = JSON.parse(parser.toJson(json));
      avance = my_content.Consolidado.Avance;
      return avance;
    } catch (e) {
...
const partidoscapitales = async () => {
  let my_content;
  try {
    const json = await readFile("./files/AVA_CJ_CA_0028_9031.xml");
    my_content = JSON.parse(parser.toJson(json));
    avance = my_content.Consolidado.Avance;
    return avance;
  } catch (e) {
